...
async function _data(FileAttachment, width, DOM) {
  ...
  for (let i = 0, n = rgba.length / 4; i < n; ++i) data[i] = Math.max(0, 1 - rgba[i * 4] / 150);
  data.width = width;
  data.height = height;
  return data;
}
...
export default function define(runtime, observer) {
  ...
  main.variable().define("script", ["require", "invalidation"], _script);
  main.variable().define("data", ["FileAttachment", "width", "DOM"], _data);
  main.variable().define("n", ["width", "height"], _n);
  main.variable().define("height", ["data"], _height);
  return main;
}
